...
async function displayCardsByIngredientTags(selectedIngredients) {
    const recipes = await getRecipes();
    const recipesDiv = document.getElementById('recipes');
    recipesDiv.innerHTML = '';
    let recipesCount = 0;
    ...
    filteredRecipes.forEach(recipe => {
        recipe.ingredients.forEach(ingredient => {
            if ((ingredient.ingredient.toLowerCase().includes(searchTagsInput)) && (!uniqueIngredients.includes(ingredient.ingredient.toLowerCase()))) {
                const ingredientName = document.createElement('p');
                ingredientName.textContent = ingredient.ingredient;
                ingredientName.addEventListener('click', async () => {
                    if (!selectedIngredients.includes(ingredient.ingredient.toLowerCase())) {
                        selectedIngredients.push(ingredient.ingredient.toLowerCase());
                        ingredients.prepend(ingredientName);
                        ingredientName.style.backgroundColor = '#FFD15B';
                        
                        ingredientName.addEventListener('mouseenter', () => {
                            const closeIcon = document.createElement('span');
                            closeIcon.textContent = '✖';
                            closeIcon.style.cursor = 'pointer';
                            ingredientName.appendChild(closeIcon);
                            ingredientName.style.fontWeight = 'bold';

                            ingredientName.addEventListener('mouseleave', () => {
                                ingredientName.removeChild(closeIcon);
                                ingredientName.style.fontWeight = 'normal';
                            });

                            closeIcon.addEventListener('click', () => {
                                const index = selectedIngredients.indexOf(ingredient.ingredient.toLowerCase());
                                if (index !== -1) {
                                    selectedIngredients.splice(index, 1);
                                    const searchInput = document.getElementById('search').value.toLowerCase();
                                    handleIngredientsTags(searchInput).then(() => {
                                        if (searchInput.length >= 3) {
                                            displayCardsBySearch();
                                        } else {
                                            displayCardsByIngredientTags(selectedIngredients);
                                        }
                                    });
                                }
                                ingredients.removeChild(ingredientName);
                            });
                            
                        });
                        await displayCardsByIngredientTags(selectedIngredients);
                    }
                });
